...
2. **Start with the two strongest competing hypotheses.** Expand beyond two only if both survive the first discriminator or the evidence exposes another serious alternative. Name specific files, functions, configs, input conditions, or invariants. Reject vague buckets ("backend issue"). If no serious alternative remains after a deliberate check, exit this differential and test or fix the sole evidenced cause directly; never fabricate a rival to continue the procedure.
