...
1. **State the symptom precisely.** Capture failing behavior, scope, timing, environment, and constraints. Separate observation from interpretation. Before proposing hypotheses, extract exact symbols, exception text, behavioral boundaries, lifecycle phases, and call-direction clues from the issue; spend the first observation on the rarest clue that can map directly to an owner.
