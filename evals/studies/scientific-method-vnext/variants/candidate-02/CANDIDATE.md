...
3. **Choose the evidence mode and name falsifiers before looking.** When tools are unavailable by task design, treat supplied issue text, traces, symbols, examples, versions, and named components as static observations; do not invent reads or tests you cannot run. For each hypothesis, name the supplied or available result that drops it and the cheapest real discriminator.
